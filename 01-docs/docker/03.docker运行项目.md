...
- 修改 初始密码账号、路径
```
-e RABBITMQ_DEFAULT_USER=dwi -e RABBITMQ_DEFAULT_PASS=dwi \       # 账号密码    
...
```
- 将 dwi.conf 文件中的所有 localhost 替换成宿主机的内网ip。 如： 192.168.1.1
- 启动
```
...
cd dwi-backend/dwi-authority/dwi-authority-server
docker build --build-arg JAR_FILE=target/dwi-authority-server.jar -t dwi-authority-server:v1 .

cd dwi-backend/dwi-msgs/dwi-msgs-server
docker build -t dwi-msgs-server:v1 .

cd dwi-backend/dwi-file/dwi-file-server
docker build -t dwi-file-server:v1 .

cd dwi-backend/dwi-gateway/dwi-zuul-server
docker build -t dwi-zuul-server:v1 .

cd dwi-backend/dwi-jobs/dwi-jobs-server
docker build -t dwi-jobs-server:v1 .
```

## 7，使用docker启动项目
```
1, 确保： nacos mysql redis 全部启动， 并且nacos 里面的redis mysql 等ip改成自己的ip （切记，使用docker时，所有的IP都不能为127.0.0.1）
2, 在nacos 里面新建  dwi-authority-server-docker.yml ,并将所有dwi-authority-server-dev.yml 里面的配置复制过去，修改docker环境和dev环境的差异部分部分。

docker run -idt  --name dwi-authority-server --restart=always \
        ...
        -p 8764:8764 \
        -v /data/:/data \
        dwi-authority-server:v1 

docker run -idt  --name dwi-zuul-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
...
docker run -idt  --name dwi-msgs-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
...
docker run -idt  --name dwi-file-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
...
docker run -idt  --name dwi-jobs-server --restart=always \
...
docker run -idt  --name dwi-nacos --restart=always \
