package com.dwi.saas.zuul.filter.pre;
...
import com.dwi.saas.zuul.filter.BaseFilter;
import com.dwi.saas.zuul.filter.MultiTenantType;
...
import static com.dwi.basic.context.ContextConstants.JWT_KEY_CLIENT_ID;
...
public class TokenContextFilter extends BaseFilter {
    ...
    @Value("${saas.database.multiTenantType:SCHEMA}")
    ...
    private boolean parseToken(RequestContext ctx, HttpServletRequest request) {
        ...
        //添加测试环境的特殊token
        if (isDev(token)) {
            authInfo = new AuthInfo().setAccount("saas").setUserId(2L)
